...
export default function RegrasBanco() {
  ...
  const [bloquearMargemNegativa, setBloquearMargemNegativa] = useState(false);
  ...
  const updateFromSimulation = useCallback(() => {
    ...
    console.log("updateFromSimulation: simulationDataStr", simulationDataStr);
    if (simulationDataStr) {
      try {
        const simulationData = JSON.parse(simulationDataStr);
        console.log("updateFromSimulation: simulationData", simulationData);
        if (simulationData.taxaJurosMensal) {
          const rate = (simulationData.taxaJurosMensal * 100).toFixed(2);
          console.log("updateFromSimulation: setting rate", rate);
  ...
  }, [isBankModalOpen, updateFromSimulation]);
  ...
  const resetBankForm = useCallback(() => {
    ...
    setBloquearMargemNegativa(false);
  ...
  const handleSaveBank = async () => {
    ...
    if (!bankName.trim()) newFieldErrors.bankName = "Nome do banco é obrigatório";
    if (!minAge) newFieldErrors.minAge = "Obrigatório";
    if (!maxAge) newFieldErrors.maxAge = "Obrigatório";
  ...
  const handleEditBank = (bank: BankRule) => {
    ...
    setBloquearMargemNegativa(bank.bloquearMargemNegativa || false);
  ...
  return (
    <div className="flex flex-col min-h-screen w-full md:max-w-none mx-auto max-w-xl bg-background-light dark:bg-background-dark text-slate-900 dark:text-slate-100 font-display pb-24 md:pb-0">
      {/* Header */}
      ...
      <main className="flex-1 w-full p-4 space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div className="bg-slate-100 dark:bg-slate-900/50 p-1 rounded-xl flex flex-wrap gap-1">
            <button
            ...
            <button
              onClick={() => setConvenioFilter('GOVERNO')}
              className={`flex-1 min-w-[80px] py-3 rounded-lg font-bold text-[10px] transition-all ${
                convenioFilter === 'GOVERNO'
                  ? 'bg-white dark:bg-slate-800 text-primary shadow-sm'
                  : 'text-slate-500 hover:bg-white/50 dark:hover:bg-slate-800/50'
              }`}
            >
            ...
            <button
              onClick={() => setConvenioFilter('FORÇAS ARMADAS')}
              className={`flex-1 min-w-[80px] py-3 rounded-lg font-bold text-[10px] transition-all ${
                convenioFilter === 'FORÇAS ARMADAS'
                  ? 'bg-white dark:bg-slate-800 text-primary shadow-sm'
                  : 'text-slate-500 hover:bg-white/50 dark:hover:bg-slate-800/50'
              }`}
            >
                  ...
                  <div className="p-4 flex items-center justify-between border-b border-slate-100 dark:border-slate-800">
                    <div className="flex items-center gap-4">
                      <div className="size-12 rounded-lg bg-slate-100 dark:bg-slate-700 flex items-center justify-center overflow-hidden relative shrink-0">
                        {bank.logoUrl ? (
                          <Image src={bank.logoUrl} alt={bank.name} fill unoptimized className="object-cover" referrerPolicy="no-referrer" />
                        ) : (
          ...
          <div className="bg-white dark:bg-slate-900 w-full max-w-md rounded-2xl shadow-xl overflow-hidden flex flex-col max-h-[85vh]">
            ...
            <div className="p-3 overflow-y-auto flex-1 min-h-0 space-y-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div className="space-y-1">
                    ...
                    <input 
                      ...
                      className={`w-full bg-slate-50 dark:bg-slate-800 border rounded-lg px-2.5 py-1.5 text-xs focus:ring-2 outline-none font-bold transition-all ${
                        fieldErrors.bankName ? 'border-red-500 focus:ring-red-500/50' : 'border-slate-200 dark:border-slate-700 focus:ring-primary/50'
                      }`} 
                      placeholder="Ex: Banco do Brasil" 
                    />
                    {fieldErrors.bankName && <span className="text-[9px] font-bold text-red-500 uppercase tracking-widest">{fieldErrors.bankName}</span>}
                  </div>
                  <div className="space-y-1">
                  ...
                  <div className="flex items-center gap-2">
                    {logoUrl && (
                      <div className="w-8 h-8 rounded-lg overflow-hidden shrink-0 border border-slate-200 dark:border-slate-700 relative">
                        <Image src={logoUrl} alt="Logo preview" fill unoptimized className="object-cover" />
                      </div>
                    )}
                  ...
                  <div className="space-y-1">
                    ...
                    <input 
                      ...
                      onChange={e => {
                        ...
                        if (fieldErrors.minAge) setFieldErrors(prev => ({...prev, minAge: undefined}));
                      }} 
                      className={`w-full bg-slate-50 dark:bg-slate-800 border rounded-lg px-2.5 py-1.5 text-xs focus:ring-2 outline-none transition-all ${
                        fieldErrors.minAge ? 'border-red-500 focus:ring-red-500/50' : 'border-slate-200 dark:border-slate-700 focus:ring-primary/50'
                      }`} 
                      placeholder="Ex: 18" 
                    />
                    {fieldErrors.minAge && <span className="text-[9px] font-bold text-red-500 uppercase tracking-widest">{fieldErrors.minAge}</span>}
                  </div>
                  <div className="space-y-1">
                    ...
                    <input 
                      ...
                      onChange={e => {
                        ...
                        if (fieldErrors.maxAge) setFieldErrors(prev => ({...prev, maxAge: undefined}));
                      }} 
                      className={`w-full bg-slate-50 dark:bg-slate-800 border rounded-lg px-2.5 py-1.5 text-xs focus:ring-2 outline-none transition-all ${
                        fieldErrors.maxAge ? 'border-red-500 focus:ring-red-500/50' : 'border-slate-200 dark:border-slate-700 focus:ring-primary/50'
                      }`} 
                      placeholder="Ex: 75" 
                    />
                    {fieldErrors.maxAge && <span className="text-[9px] font-bold text-red-500 uppercase tracking-widest">{fieldErrors.maxAge}</span>}
                  </div>
                  <div className="space-y-1">
              ...
              <div className="space-y-3">
                ...
                <label className="flex items-center gap-2 p-2 border border-slate-200 dark:border-slate-700 rounded-lg cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-800/50">
                  <input type="checkbox" checked={bloquearMargemNegativa} onChange={e => setBloquearMargemNegativa(e.target.checked)} className="w-4 h-4 rounded border-slate-300 text-primary focus:ring-primary" />
                  <span className="text-xs font-medium">Bloquear Simulação se houver Margem Negativa</span>
                </label>

                <label className="flex items-center gap-2 p-2 border border-slate-200 dark:border-slate-700 rounded-lg cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-800/50">
                  <input type="checkbox" checked={accepts60Mais} onChange={e => setAccepts60Mais(e.target.checked)} className="w-4 h-4 rounded border-slate-300 text-primary focus:ring-primary" />
