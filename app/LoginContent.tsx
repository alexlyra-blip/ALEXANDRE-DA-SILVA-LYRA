...
import { getBrandingSettings, getBrandingBySlug } from '@/lib/data-service';
...
export default function LoginContent() {
  ...
  // Debounced branding lookup
  useEffect(() => {
    if (!email) return;

    const handler = setTimeout(async () => {
      try {
        console.log("LoginContent: Fetching branding for email...", email);
        // This is a simplified lookup, maybe need a specific service function
        // For now, let's just log and see. 
        // Need to add a lookup function in data-service.ts if not exists.
      } catch (error) {
        console.error("LoginContent: Error looking up branding by email", error);
      }
    }, 1000);
      ...
      try {
        ...
        if (data) {
          const brandingData = {
            ...
            promoterName: (['Alexandre Lyra', 'Alexndre Lyra'].includes(data.promoterName) ? 'Portabilidade PRO' : data.promoterName) || 'Portabilidade PRO'
          };
          console.log("LoginContent: Branding settings loaded:", brandingData);
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error("LoginContent: Login failed", error);
  ...
  const handleResetPassword = async () => {
    const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
    if (!email) {
      setFieldErrors({ email: "Insira seu e-mail para recuperar a senha" });
      return;
    }
  ...
  if (!isAuthReady) {
    return (
      <div className="flex flex-col h-screen w-full max-w-md mx-auto bg-background items-center justify-center">
  ...
  return (
    <div className="flex flex-col h-full min-h-screen w-full max-w-md mx-auto bg-background overflow-x-hidden shadow-2xl">
      ...
      <div className="flex items-center p-4 pb-2 justify-between">
        <div className="text-foreground flex size-12 shrink-0 items-center justify-center cursor-pointer">
      ...
      <form onSubmit={handleLogin} className="px-4 py-4 flex flex-col gap-4">
        ...
        <div className="flex flex-col gap-1">
          ...
          <div className="relative">
            <User className={`absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 transition-colors ${fieldErrors.email ? 'text-red-500' : 'text-slate-400'}`} />
            <input
              type="email"
              value={email}
              onChange={(e) => {
              ...
              className={`w-full bg-white dark:bg-slate-900 border rounded-xl py-4 pl-12 pr-4 text-slate-900 dark:text-black focus:outline-none focus:ring-2 transition-all ${
        ...
        <div className="flex flex-col gap-1">
          ...
          <div className="relative">
            ...
            <input
              ...
              placeholder="••••••••"
              className={`w-full bg-white dark:bg-slate-900 border rounded-xl py-4 pl-12 pr-12 text-slate-900 dark:text-black focus:outline-none focus:ring-2 transition-all ${
