import { ref, uploadBytesResumable, uploadBytes, getDownloadURL, UploadTaskSnapshot, deleteObject } from 'firebase/storage';
 ...
 * @param urlOrPath The download URL or the path in storage.
 */
export const deleteFile = async (urlOrPath: string | null | undefined): Promise<void> => {
  if (!urlOrPath) return;
  
  try {
    console.log(`[StorageService] Attempting to delete file: ${urlOrPath}`);
    const storageRef = ref(storage, urlOrPath);
    await deleteObject(storageRef);
    console.log(`[StorageService] File deleted successfully: ${urlOrPath}`);
  } catch (error: any) {
    // If the file doesn't exist, we don't need to throw an error
    if (error.code === 'storage/object-not-found') {
      console.warn(`[StorageService] File not found for deletion: ${urlOrPath}`);
      return;
    }
    console.error(`[StorageService] Error deleting file: ${urlOrPath}`, error);
    throw error;
  }
};
 ...
 */
/**
 * Converts a gs:// URL to a public https:// download URL if possible.
 * If it's already an https URL or not a gs URL, returns it as is.
 */
export const getPublicUrl = (url: string | null | undefined): string => {
  if (!url) return '';
...
export const uploadFileViaApi = async (
  ...
  try {
    ...
    const responseText = await response.text();
    console.log(`[StorageService ${requestId}] Response text:`, responseText);
    let data;
    try {
